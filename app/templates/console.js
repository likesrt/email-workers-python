(function () {
  ...
  const ROW_POLL_INTERVAL_MS = 1800;
  const ROW_POLL_MAX_ATTEMPTS = 20;
  ...
  const state = {
    ...
    lastItems: [],
    autoRefreshTimer: 0,
    autoRefreshCountdownTimer: 0,
    autoRefreshRemainingSeconds: 0,
    autoCleanupCountdownTimer: 0,
    autoCleanupRemainingSeconds: 0,
    isAutoRefreshOn: true,
    isAutoCleanupOn: false,
    isLoadingMails: false,
    isCleaningUp: false,
    autoCleanupConfiguredMinutes: 10,
    autoCleanupLastRunAt: "",
    autoCleanupLastDeletedCount: 0,
    detailBlobUrls: [],
    rowPollTimers: {},
    rowPollAttempts: {}
  };
  ...
  function stopRowPolling(mailId) {
    const timer = state.rowPollTimers[mailId];
    if (timer) clearTimeout(timer);
    delete state.rowPollTimers[mailId];
    delete state.rowPollAttempts[mailId];
  }

  function stopAllRowPolling() {
    Object.keys(state.rowPollTimers).forEach(stopRowPolling);
  }
  ...
  function sanitizeHtml(value) {
    ...
    doc.querySelectorAll("*").forEach(function (node) {
      Array.from(node.attributes).forEach(function (attr) {
        ...
        if (["src", "href", "xlink:href"].includes(name) && attrValue.startsWith("javascript:")) {
  ...
  function buildHtmlPreviewDocument(value) {
    ...
    return [
      ...
      '<style>html,body{margin:0;padding:0;background:#fff;color:#0f172a;font-family:"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif;font-size:14px;}body{padding:12px;line-height:1.5;}img{max-width:100%;height:auto;}table{max-width:100%;border-collapse:collapse;}pre{white-space:pre-wrap;word-break:break-word;}a{color:#2563eb;}</style>',
      '</head><body>',
      cleanHtml,
      '</body></html>'
    ].join("");
  ...
  function getExtractionStatusMeta(item) {
    const status = String((item && item.extractionStatus) || "idle");
    if (status === "failed") return { label: "识别失败", kind: "failed" };
    if (status === "done") return { label: item && item.verificationCode ? "已识别" : "无验证码", kind: "done" };
    if (status === "pending") return { label: "识别中", kind: "pending" };
    return { label: "未识别", kind: "idle" };
  }
  ...
  function renderActionCell(item) {
    const retryDisabled = String(item && item.extractionStatus || "") === "pending";
    return [
      '<td class="col-actions"><div class="action-stack">',
      '<button class="btn btn-secondary detail-btn" type="button" data-id="', escapeHtml(item.id || ""), '">详情</button>',
      '<button class="btn btn-secondary retry-extraction-btn" type="button" data-id="', escapeHtml(item.id || ""), '"', retryDisabled ? ' disabled' : '', '>', retryDisabled ? '识别中' : '重试', '</button>',
      '</div></td>'
    ].join("");
  }
  ...
  function findItemIndexById(mailId) {
    return state.lastItems.findIndex(function (item) {
      return String(item && item.id || "") === String(mailId || "");
    });
  }

  function updateRowItem(mailId, nextItem) {
    const index = findItemIndexById(mailId);
    if (index < 0) return;
    state.lastItems[index] = { ...state.lastItems[index], ...nextItem };
  ...
  function renderTable(items) {
    ...
    const rows = items.map(function (item) {
      const codeCell = item.verificationCode
        ...
        : '<td class="col-code font-mono"><span class="small">-</span></td>';
  ...
  async function fetchMailSummary(mailId) {
    return await fetchJson("/api/mails/" + encodeURIComponent(mailId) + "/summary", { method: "GET" });
  }

  function scheduleMailSummaryPoll(mailId) {
    const nextAttempt = Number(state.rowPollAttempts[mailId] || 0) + 1;
    state.rowPollAttempts[mailId] = nextAttempt;
    stopRowPollingTimerOnly(mailId);
    state.rowPollTimers[mailId] = window.setTimeout(async function () {
      try {
        const item = await fetchMailSummary(mailId);
        updateRowItem(mailId, item);
        if (String(item.extractionStatus || "") === "pending" && nextAttempt < ROW_POLL_MAX_ATTEMPTS) {
          scheduleMailSummaryPoll(mailId);
          return;
        }
        if (String(item.extractionStatus || "") === "pending") {
          stopRowPolling(mailId);
          setActionStatus("当前邮件仍在识别中，已停止轮询，请稍后手动查询。", "info");
          return;
        }
        stopRowPolling(mailId);
        setActionStatus("当前邮件识别状态已更新。", "success");
      } catch (error) {
        stopRowPolling(mailId);
        setActionStatus("单条状态刷新失败: " + (error && error.message ? error.message : String(error)), "error");
      }
    }, ROW_POLL_INTERVAL_MS);
  }

  function stopRowPollingTimerOnly(mailId) {
    const timer = state.rowPollTimers[mailId];
    if (!timer) return;
    clearTimeout(timer);
    delete state.rowPollTimers[mailId];
  ...
  async function retryExtraction(id) {
    try {
      ...
      stopRowPolling(id);
      const item = await fetchJson("/api/mails/" + encodeURIComponent(id) + "/retry-extraction", { method: "POST" });
      updateRowItem(id, item);
      state.rowPollAttempts[id] = 0;
      scheduleMailSummaryPoll(id);
      setActionStatus("已提交后台识别，当前行会自动刷新。", "success");
    } catch (error) {
  ...
  mailTableBody.addEventListener("click", function (event) {
    ...
    if (retryButton instanceof HTMLElement) {
      ...
      if (id && !retryButton.disabled) retryExtraction(id);
      return;
    }
